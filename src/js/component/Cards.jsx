...
const Cartas = (props) => {
      ...
      <div className="card" style={{ backgroundColor: '#0d1117', color: 'white' }}>
        ...
        <div className="card-body px-2 py-2 fw-semibold">
          ...
          <div className="d-flex justify-content-between">
            <Link to={`/${type}_details/${item.uid}`} className="boton-leer btn-outline-primary fw-bold">Leer más</Link>
            <button 
              onClick={() => actions.AddOrDeleteFavorite({...item, type})} 
              className="boton-favorito btn-outline-warning"
            >
              <i 
  ...
  return (
    <>
      <h1 className=" mb-2 sub-title">PERSONAJES</h1>
      <div className="row flex-nowrap overflow-auto">
        {store.personajes.map(item => renderCard(item, 'character'))}
      </div>

      <h1 className=" mb-2 mt-4 sub-title">PLANETAS</h1>
      <div className="row flex-nowrap overflow-auto">
        {store.planets.map(item => renderCard(item, 'planet'))}
      </div>

      <h1 className=" mb-2 mt-4 sub-title">VEHICULOS</h1>
      <div className="row flex-nowrap overflow-auto">
